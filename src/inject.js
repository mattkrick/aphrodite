...
const injectStyleTag = () => {
  if (!styleTag) {
    ...
    if (!styleTag) {
      ...
      const cssContents = injectionBuffer.join('');
      styleTag.appendChild(document.createTextNode(cssContents));
      head.appendChild(styleTag);
    }
  } else {
    // Add only vendor-prefixed rules without forcing a re-evaluation of the styles
    for (let i = 0; i < injectionBuffer.length; i++) {
      const rule = injectionBuffer[i];
      try {
...
const atRuleHandlers = {
  ...
  animationName: (val) => {
    ...
    Object.keys(val).forEach(key => {
      finalVal += generateCSS(key, [val[key]], atRuleHandlers, false);
    });
    finalVal += '}';
...
export const injectStyleOnce = (key, selector, rawRulesets) => {
  if (!alreadyInjected[key]) {
    const generated = generateCSS(selector, rawRulesets, atRuleHandlers);
...
export const injectAndGetClassName = (styleDefinitions) => {
  ...
  // `css(a, test && c)`
  const validDefinitions = styleDefinitions.filter(Boolean);
